...
from onnxocr.rapid_doc.utils.enum_class import BlockType
from onnxocr.rapid_doc.utils.pdf_reader import image_to_b64str
...
class XlsxConverter:
    ...
    def convert(
        ...
        if self.workbook is not None:
            ...
            for idx, name in enumerate(self.workbook.sheetnames):
                ...
                if not self._should_process_sheet(sheet):
                    logger.info(
                        f"跳过第 {idx + 1} 个工作表：{name}（隐藏工作表）"
                    )
                    continue
    ...
    def _should_process_sheet(self, sheet: Worksheet) -> bool:
        """根据配置判断是否处理当前工作表。"""
        return self.include_hidden_sheets or (
            sheet.sheet_state == Worksheet.SHEETSTATE_VISIBLE
        )
