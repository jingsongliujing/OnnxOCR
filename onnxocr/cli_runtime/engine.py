...
from .base import OcrLine, CLIInput
from .extractors import normalize_onnxocr_result


class OnnxOCREngine:
    """Lazy ONNXPaddleOcr adapter used by vertical CLI scenarios."""
    ...
    def recognize(self, cli_input: CLIInput) -> List[OcrLine]:
        if cli_input.ocr_lines is not None:
            return cli_input.ocr_lines
        img = self._load_image(cli_input)
    ...
    @staticmethod
    def _load_image(cli_input: CLIInput):
        if cli_input.image is not None:
            return cli_input.image
        if not cli_input.image_path:
            raise ValueError("CLIInput requires image or image_path.")
        img = cv2.imread(cli_input.image_path)
        if img is None:
            raise ValueError(f"Failed to read image: {cli_input.image_path}")
        return img
